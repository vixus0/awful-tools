...
class Mdrunr(object):
    ...
    def add_job(self, job):
        ...
        with self.running_or_done_queue.locked():
            with self.todo_queue.locked():
                new_name,changed = self._check_name(job.name)
                if changed: 
                    logging.debug("Name change: "+job.name+" -> "+new_name)
                    job.change_name(new_name)
    ...
    def _check_name(self, basename):
        """
        Check that `basename` does not conflict with existing job names. If it does,
        suggest a new name.

        :Returns: 
            - `new_name` : a suggestion for a job name.
            - `changed` : True if `new_name != basename`, ie. there was a conflict.

        The new names are generated by appending '-x' to `basename`, where `x` is the
        smallest integer that will not cause conflict.
        """
        existing_names = self.todo_queue.keys() + self.running_or_done_queue.keys()
        suffix = ""
        job_number = 0
        changed = False
        while basename+suffix in existing_names:
            job_number += 1
            suffix = "-"+str(job_number)
            changed=True
        return basename+suffix, changed
